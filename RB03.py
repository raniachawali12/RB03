...
form_responses_path = "RB03.txt"  # Remplacez par le chemin de votre fichier texte ou laissez None
...
# ---------------------------------------------------------------------
# 7. Fonction de sauvegarde de la conversation
# ---------------------------------------------------------------------
def save_conversation():
    """Sauvegarde la conversation dans un fichier JSON avec un nom basé sur la date et l'heure."""
    conversation = st.session_state.messages
    now = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"RB03_chat/conversation_{now}.json"
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(conversation, f, ensure_ascii=False, indent=4)
    return filename

# ---------------------------------------------------------------------
# 8. Interface Streamlit façon "chat" avec st.chat_message et st.chat_input
# ---------------------------------------------------------------------
...
if st.session_state.messages:
    conversation_json = json.dumps(st.session_state.messages, ensure_ascii=False, indent=4)
    st.download_button(
...
if user_input := st.chat_input("Posez votre question…"):
    # Si l'utilisateur tape "exit", on sauvegarde la conversation et on arrête l'application
    if user_input.strip().lower() == "exit":
        filename = save_conversation()
        st.success(f"Conversation sauvegardée dans le fichier {filename}. Vous pouvez fermer la fenêtre.")
        st.stop()
    else:
